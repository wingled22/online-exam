...
class RoomDB
{
	...
	protected function activateRoom($room_id){
		$query_string ="UPDATE rooms SET active = '1' WHERE room_id = ".$room_id ;	 
		$conn = $this->connectDb();		 
		if ($conn->connect_error) {
  		  die("Connection failed: " . $conn->connect_error);
		}else{
	...
	protected function deactivateRoom($room_id){
		$query_string ="UPDATE rooms SET active = NULL WHERE room_id = ".$room_id ;	 
		$conn = $this->connectDb();		 
		if ($conn->connect_error) {
  		  die("Connection failed: " . $conn->connect_error);
		}else{
